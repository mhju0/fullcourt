...
| `src/lib/__tests__/design-scale.test.ts` | Pins `TYPE` and `SPACE`: the eight type steps by name (a diff shows which one moved), that no step is fractional, that the label ladder `micro < label < data` cannot invert, and that the shared table styles are built from the tokens rather than literals — those three reach all 21 tables through `DataTable`. Also reads `scripts/audit_design_scale.mjs` and asserts **its** copy of both scales matches: the script is plain node so it cannot import the module, and a stale copy would report a clean sheet while the app drifted. Discriminating: smuggling a 14 into the script's `TYPE_SCALE` fails exactly that test, verified. |
