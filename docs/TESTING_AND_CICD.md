...
| `src/components/__tests__/referee-effect-content.test.ts` | Renders the finished, unpublished Referee Effect copy, which nothing else does — without it a broken prop or a missing space would surface on the day it is published. Asserts the attribution caveat, the make-up-call sign flip (published without it, `t = 27` reads as proof of compensation), and that no extreme pair appears without the chance count beside it. |
