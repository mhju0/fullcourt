...
- The original base tables (`teams`, `games`, `fatigue_scores`, `predictions`) predate the
  committed migration history. Do **not** run `drizzle-kit push` or `generate` against an
  existing FullCourt database: `schema.ts` intentionally lags the live schema. Incremental
  changes ship as **hand-applied, standalone** SQL files pasted into the Supabase SQL editor:
  `playoff_series`
...
teams (1) ──────< playoff_series.home_court_team_id    (FK)   ┐ Playoff Predictor
...
| `0006_playoff_series.sql` | Create `playoff_series` (Playoff Predictor) + its RLS policies and Data API grants (mirrors `0004`/`0005`). Standalone SQL — not generated by `drizzle-kit`. |
