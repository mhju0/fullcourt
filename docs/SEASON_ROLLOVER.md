...
| `src/components/about-content.tsx:185`, `:194`, `:199` | "38,985 games", "+13.4", "26" | Frozen marketing copy on `/about`. The season count beside them **does** derive (`NBA_SEASONS.length`), which makes the hardcoded figures look derived too — they are not. The game total grows every season; `+13.4` is the RA≥7 deviation and moves as the backtest grows; `26` is last season's rest spread and changes annually. Left static on purpose: `/about` fetches nothing, and giving it a data dependency to keep three decorative numbers fresh is a bad trade. Re-read them off `/analysis` and `/schedule` at rollover. |
