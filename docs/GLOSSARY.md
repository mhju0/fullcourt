...
| REFEREE EFFECT *(OTHER)* | `/referees` | What each official calls | Referee Bias — the page was named for the question, not a conclusion, and the question came back inside noise; since the 2026-07-31 rebuild it publishes foul *style* (each official's mix of calls against the league's own seasonal mix), which is explicitly not a fairness claim |
